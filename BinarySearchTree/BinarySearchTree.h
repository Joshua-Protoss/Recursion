...
class BinarySearchTree : public Tree<t_Data>
{
    ...
    // every nodes have a left child and right and the data of course
    struct Node
    {
        t_Data data;
...
public:
    ...
    void traversal() const override {
        // if the root doess not exist we return
        if (!root)
            return;
...
private:
    ...
    // in-order traversal: visit left subtree then root then left subtree
    ...
    // get the previous node in the in-oder traversal order (so the largest node in the left subtree)
