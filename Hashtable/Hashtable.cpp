...
int HashTable::get(int key) {
    ...
    // maybe there are already items with this index (the vector stores these items)
    ...
    // Search for the item [O(N) linear running time complexity]
...
void HashTable::put(int key, int value) {
    ...
    // if the list is empty : it means that this is the first item with this index
    if(items.empty()){
        std::cout << "No collision just insertion... \n";
    } else{
        // the list is not empty: there are already items with this key (collision)
        std::cout << "Collision when inserting with key " << key << '\n';
    ...
    // insert the new items to this array index
    items.push_back(HashItem{key, value});
    std::cout << "Size of the list is: " << items.size() << '\n';
