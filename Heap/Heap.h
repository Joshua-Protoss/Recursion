...
public:
    ...
    // heap properties are not violated (O(logN)) using the fixUp() method
    ...
    // note: we just do not care about that item anymore but because
...
private:
    // we consider the last item and checks wether swaps are needed or not
    // running time: O(logN)
    void fixUp(int index);

    // we have a given item in the heap and we consider all the item BELOW and check
    // wether the heap properties are violated or not
    void fixDown(int index);

    // checks wether the heap is full or not
