...
class LinkedList : public List<t_Data>
{
    ...
    public:
        ...
        void remove(const t_Data &data) override
        {
            ...
            std::unique_ptr<Node> temp = std::move(next->nextNode); // grab data from the next node of the node that we want to delete
            prev->nextNode = std::move(temp);   // prev->nextNode is equal to the node that we want to delete, we replace that nodde with next->nextNode, essentially the node after the node that we want to delete
        }
